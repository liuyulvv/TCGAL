...
pub struct Neighbors<T: NumberType> {
    pub left: Option<Rc<RefCell<Edge2<T>>>>,
    pub right: Option<Rc<RefCell<Edge2<T>>>>,
    phantom: PhantomData<T>,
}

pub struct PEdges<T: NumberType> {
    pub u: Vec<Rc<RefCell<Edge2<T>>>>,
    pub l: Vec<Rc<RefCell<Edge2<T>>>>,
    pub c: Vec<Rc<RefCell<Edge2<T>>>>,
}

#[derive(Debug, Clone)]
pub struct StatusStructure<T: NumberType> {
    vertex_vec: Vec<T>,
    edge_vec: Vec<Rc<RefCell<Edge2<T>>>>,
}

impl<T: NumberType> StatusStructure<T> {
    pub fn new() -> Self {
        Self {
            vertex_vec: Vec::new(),
            edge_vec: Vec::new(),
    ...
    pub fn insert(&mut self, edge: Rc<RefCell<Edge2<T>>>) -> Option<Neighbors<T>> {
        ...
        match insert_index {
            Some(index) => {
                ...
                match (left, right) {
                    ...
                    (None, Some(right)) => Some(Neighbors::<T> {
                        left: None,
                        right: Some(right.clone()),
                        phantom: PhantomData,
                    }),
    ...
    pub fn remove(&mut self, edge: Rc<RefCell<Edge2<T>>>) -> Option<Neighbors<T>> {
        let remove_index = self.edge_vec.iter().position(|e| {
            let e_binding = e.borrow();
            let edge_binding: std::cell::Ref<Edge2<T>> = edge.borrow();
    ...
    pub fn get_p_edges(&self, p: Rc<RefCell<Vertex2<T>>>) -> PEdges<T> {
    ...
    pub fn calculate_insert_index(&mut self, edge: Rc<RefCell<Edge2<T>>>) -> Option<usize> {
        let edge_binding = edge.borrow();
        let x = edge_binding.source().borrow().x();
        self.vertex_vec.iter().position(|e| e >= &x)
    }

    pub fn update_x(&mut self, edge: Rc<RefCell<Edge2<T>>>) {
        let edge_binding = edge.borrow();
        let source = edge_binding.source();
    ...
    fn calculate_x(edge: Rc<RefCell<Edge2<T>>>, y: T) -> T {
        let edge_binding = edge.borrow();
        let source = edge_binding.source();
        let target = edge_binding.target();
